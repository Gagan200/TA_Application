...
def is_instructor(user):
    return user.user_type == user.INSTRUCTOR
...
def home(request):
    ...
    application_list = Application.objects.filter(status=Application.APPROVED)

    context = {
        'application_list': application_list,
    }
    return render(request, "instructors/home.html", context)
...
def check_application(request, application_id):
    ...
    if request.method == "POST":
        if "like" in request.POST:
            application.liked_by.add(request.user)
            messages.success(request, "Your response was recorded!")

        if "dislike" in request.POST:
            application.disliked_by.add(request.user)
            messages.success(request, "Your response was recorded!")
        ...
        if "notify" in request.POST:
            send_email(
                ...
                message=f"Congratulations! Your application for {application.course} at North University has been approved."
            )
    ...
    comment_list = Comment.objects.filter(application=application).order_by("-date")
    context = {
    ...
    return render(request, "instructors/check_application.html", context)
